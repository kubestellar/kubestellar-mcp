...
closed without the underlying fix landing (#851 was itself auto-closed by its
own fix PR's `Fixes #851` keyword, which resolved only the doc half); see
kubestellar-mcp#854, which tracks adding an automated `if: failure()` alert
step. Until that lands, check for this manually:
