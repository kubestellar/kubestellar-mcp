...
closed without the underlying fix landing (each was closed by a doc-only PR's
auto-close keyword before the `.github/workflows/release.yml` fix itself
landed); see kubestellar-mcp#865, the current tracker, which carries forward
the drafted `if: failure()` alert step. Until that lands, check for this
manually:
