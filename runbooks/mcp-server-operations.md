...
for `release.yml`, all the same gap class — #730, #753, and #783 are all
closed as their interim-runbook deliverables shipped; the underlying
`release.yml` `if: failure()` alert step itself is still outstanding. Its
tracker has been reopened repeatedly (#694 → #771 → #783 → #851, each closed
by a doc-only PR's auto-close keyword before the workflow fix landed); #865
is the current tracker — see it before closing this reference again). A
failed scheduled run is visible
