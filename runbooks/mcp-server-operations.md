...
for `stale.yml`, and [#854](https://github.com/kubestellar/kubestellar-mcp/issues/854)
for `release.yml`, all the same gap class — #730, #753, #783, and #851 are all
closed as their interim-runbook deliverables shipped or (for #851) auto-closed
by a closing keyword before the fix landed; the underlying `release.yml`
`if: failure()` alert step itself is still outstanding and is tracked in
#854). A failed
