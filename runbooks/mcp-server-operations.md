...
all closed without the underlying `release.yml` `if: failure()` alert step
landing (see #865 for why: a doc-only PR's auto-close keyword closed each
prior tracker in turn); the fix itself is still outstanding and is tracked
in #865). A failed
