...
func TestValidateHelmChartRef(t *testing.T) {
	tests := []struct {
		...
		{"blocks nested path traversal", "foo/../../../etc/passwd", true},
		{"blocks mid-path parent traversal", "charts/../../evil", true},
		{"blocks flag injection post-renderer", "--post-renderer", true},
