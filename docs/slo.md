...
The MCP server's primary transport is stdio, not HTTP, and no metrics endpoint is exposed by default. SLO compliance is assessed via:
...
- **Note on SLO 2 specifically:** `mcpserver_tool_duration_seconds` measures latency across *all* tool calls, not the narrower "`initialize` receipt to first `tools/list` response" SLI that SLO 2 defines. The `MCPServerHighToolLatencyP95` alert in `docs/alerts/mcpserver-rules.yaml` is therefore a general tool-latency proxy, not a direct measurement of SLO 2. No metric currently isolates discovery/handshake latency specifically; adding one is a suggestion only, left to a maintainer to prioritize.
